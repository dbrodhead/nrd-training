class TrainingDaysController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @training_day.save
        ...
        format.json { render action: 'show', status: :created, location: @training_day }
      else
  ...
  def update
    respond_to do |format|
      if @training_day.update(training_day_params)
        ...
        format.json { render action: 'show', status: :ok, location: @training_day }
      else
  ...
  private
    ...
    def training_day_params
      params.require(:training_day).permit(:date, :details)
    end
end
